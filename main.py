...
class RunAllEolHostingIngestions(luigi.WrapperTask):
    data_folder = luigi.Parameter()
    eol_tables_config_path = luigi.Parameter()
    environments_to_ingest = luigi.ListParameter()
    ...
    def requires(self):
        ...
        for environment, files_to_ingest_for_environement in groupby(configured_files_to_ingest, lambda x: x.Environment):
            if (not environment in self.environments_to_ingest):
                continue
            for file_to_ingest in files_to_ingest_for_environement:
                ...
                yield IngestParquetFile(
                    ...
                    environment = file_to_ingest.Environment,
    ...
    def check_path(self, file_path):
        # path structure: data folder path\\environment=NL\\Tables\\2019\\12\\31
        return len(file_path.split('\\')[-5:][0].split('=')) > 1
...
class CigIngestionFile():
    def __init__(self, file_path):
        ...
        # path structure: data folder path\\environment=NL\\2019\\12\\31\\file.parquet
        ...
        environment = splitted_path[0].split('=')[1]
...
if __name__ == "__main__":
    try:
        ...
        environments_to_ingest = config['environments']
        ...
        execution_summary = luigi.build(
            [RunAllEolHostingIngestions(
                eol_tables_config_path = tables_to_upload_config_file,
                data_folder = data_folder,
                environments_to_ingest = environments_to_ingest,
                ingest_to = ingest_to,
        ...
        if config['is_execution_summary_via_slack_enabled'] and (
            ...
            not execution_summary.worker.run_succeeded):
                send_execution_summary(execution_summary, environments_to_ingest, ingest_to, ingestion_date)

    except Exception as e:
        message = ":rage4: \n EOL Hosting ETL : Unexpected failure \nError:\n{}\n{}"\
