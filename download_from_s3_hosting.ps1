param ([string]$ingestion_config_filename = "ingestion_config.json")
...
$environments = $ingestion_config.environments
...
$logfile = $ingestion_config.logs_folder+"\download_from_s3_hosting."+$today.tostring("yyyy-MM")+".log"
...
try {
    ...
    {
        LogWrite "`nDownloading Environment $environment`n"
        foreach($table in $json)
        {
            ...
            $location = "environment=$environment/$tablename/$year/$month/$day"
